const NT = (function () {
  ...
  function render() {
    ...
    const editorHtml = note
      ? `<div class="nt-editor"><div class="nt-editor-head"><input class="nt-editor-title" value="${esc(note.title)}" placeholder="Untitled" onchange="NT.saveTitle(this.value)" onkeydown="if(event.key==='Enter')this.blur();"><div class="nt-meta-row"><input class="ti nt-editor-tags" value="${esc(note.tags || '')}" placeholder="tags, comma separated" onchange="NT.saveTags(this.value)"><button class="nt-editor-pin${note.pinned ? ' active' : ''}" onclick="NT.togglePin()">${note.pinned ? 'Pinned' : 'Pin'}</button><button class="nt-editor-del" onclick="NT.deleteNote()">Delete</button></div></div>${window.WSLinks ? WSLinks.renderPanel({ app: 'notes', id: note.id }) : ''}<div class="nt-editor-body"><textarea placeholder="Start writing..." oninput="NT.saveBody(this.value)">${esc(note.body || '')}</textarea></div></div>`
      : `<div class="nt-empty">Select a note or create a new one</div>`;

    root.innerHTML = `<div class="nt-topbar"><div class="app-title">Notes</div></div><div class="nt-wrap"><div class="nt-sidebar"><div class="nt-search nt-search-row"><input placeholder="Search notes..." value="${esc(searchTerm)}" oninput="NT.search(this.value)"><button class="nt-sidebar-add" onclick="NT.create()">+</button></div><div class="nt-search" style="padding-top:8px">${tagFilters}</div><div class="nt-list">${listHtml || '<div class="nt-empty" style="padding:22px 14px">No notes found</div>'}</div></div>${editorHtml}</div>`;
  }
